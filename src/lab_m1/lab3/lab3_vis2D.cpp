...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    float dist = 0.1 * deltaTime;
    if (window->KeyHold(GLFW_KEY_W)) {
        logicSpace.y = logicSpace.y + dist;
    }
    ...
    if (window->KeyHold(GLFW_KEY_A)) {
        logicSpace.x = logicSpace.x - dist;
    }
    ...
    if (window->KeyHold(GLFW_KEY_Z)) {
        logicSpace.width = logicSpace.width - dist;
        logicSpace.height = logicSpace.height - dist;
    }
